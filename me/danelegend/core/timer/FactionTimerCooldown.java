...
public class FactionTimerCooldown {

	private final Timer timer;
	private final String owner; // the id of faction that this timer runnable is for
	...
	public FactionTimerCooldown(Timer timer, long duration) {
		this.owner = null;
		this.timer = timer;
		this.setRemaining(duration);
	}

	public FactionTimerCooldown(Timer timer, String factionId, long duration) {
		this.timer = timer;
	...
	private void setExpiryMillis(long remainingMillis) {
		...
		if (remainingMillis > 0L) {
			...
			this.eventNotificationTask = new BukkitRunnable() {
				@Override
				public void run() {
					Bukkit.getPluginManager().callEvent(new FactionTimerExpireEvent(FactionTimerCooldown.this.owner, FactionTimerCooldown.this.timer));
				}
			}.runTaskLater(P.p, remainingMillis / 50L);
		}
	}
