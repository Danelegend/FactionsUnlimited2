...
public class FactionsChatListener implements Listener {
	...
	public void onPlayerEarlyChat(AsyncPlayerChatEvent event) {
		...
		if (chat == ChatMode.FACTION) {
			...
			Bukkit.getLogger().log(Level.INFO,
					ChatColor.stripColor("FactionChat " + myFaction.getTag() + ": " + message));
			...
			for (FPlayer fp : myFaction.getFPlayers()) {
				if (fp.getRole().isAtLeast(Role.MODERATOR)) {
			...
			Bukkit.getLogger().log(Level.INFO, ChatColor.stripColor("ModChat: " + message));
			event.setCancelled(true);
		}
	}
	...
	public void onPlayerChat(AsyncPlayerChatEvent event) {
		...
		if (!Conf.chatTagReplaceString.isEmpty() && eventFormat.contains(Conf.chatTagReplaceString)) {
			...
			if (me.getFaction().isWilderness()) {
				eventFormat = eventFormat.replace(Conf.chatTagReplaceString, ChatColor.RED + "-");
			} else {
		...
		String formatStart = eventFormat.substring(0, InsertIndex)
				+ ((Conf.chatTagPadBefore && !me.getChatTag().isEmpty()) ? " " : "");
		String formatEnd = ((Conf.chatTagPadAfter && !me.getChatTag().isEmpty()) ? " " : "")
		...
		if (Conf.chatTagRelationColored) {
			...
			for (Player listeningPlayer : event.getRecipients()) {
				...
				try {
					...
					P.p.log(Level.SEVERE,
							"NOTE: This has been automatically fixed right now by setting chatTagInsertIndex to 0.");
					P.p.log(Level.SEVERE,
			...
			String nonColoredMsg = ChatColor
					.stripColor(String.format(nonColoredMsgFormat, talkingPlayer.getDisplayName(), msg));
