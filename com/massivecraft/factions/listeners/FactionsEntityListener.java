...
public class FactionsEntityListener implements Listener {
	...
	public void onEntityDeath(EntityDeathEvent event) {
		...
		} else if (faction.isWilderness() && !Conf.wildernessPowerLoss
	 ...
	 * enemies. I can hurt neutrals as long as they are outside their own territory.
	 */
	...
	public void onEntityExplode(EntityExplodeEvent event) {
		...
		// TODO: :(
		if (boomer instanceof Creeper && ((faction.isWilderness() && Conf.wildernessBlockCreepers
				&& !Conf.worldsNoWildernessProtection.contains(loc.getWorld().getName()))
				|| (faction.isNormal()
						&& (online ? Conf.territoryBlockCreepers : Conf.territoryBlockCreepersWhenOffline))
				|| (faction.isWarZone() && Conf.warZoneBlockCreepers) || faction.isSafeZone())) {
			// creeper which needs prevention
			event.setCancelled(true);
		} else if (
		// it's a bit crude just using fireball protection for Wither boss too, but I'd
		// rather not add in a whole new set of xxxBlockWitherExplosion or whatever
		(boomer instanceof Fireball || boomer instanceof WitherSkull || boomer instanceof Wither)
				&& ((faction.isWilderness() && Conf.wildernessBlockFireballs
						&& !Conf.worldsNoWildernessProtection.contains(loc.getWorld().getName()))
						|| (faction.isNormal()
								&& (online ? Conf.territoryBlockFireballs : Conf.territoryBlockFireballsWhenOffline))
						|| (faction.isWarZone() && Conf.warZoneBlockFireballs) || faction.isSafeZone())) {
			// ghast fireball which needs prevention
			event.setCancelled(true);
		} else if ((boomer instanceof TNTPrimed || boomer instanceof ExplosiveMinecart) && ((faction.isWilderness()
				&& Conf.wildernessBlockTNT && !Conf.worldsNoWildernessProtection.contains(loc.getWorld().getName()))
			...
			// TNT in water/lava doesn't normally destroy any surrounding blocks, which is
			// usually desired behavior, but...
			// this change below provides workaround for waterwalling providing perfect
			// protection,
			// and makes cheap (non-obsidian) TNT cannons require minor maintenance between
			// shots
			Block center = loc.getBlock();
			if (center.isLiquid()) {
				// a single surrounding block in all 6 directions is broken if the material is
				// weak enough
				...
				for (Block target : targets) {
					...
					// ignore air, bedrock, water, lava, obsidian, enchanting table, etc.... too bad
					// we can't get a blast resistance value through Bukkit yet
	...
	// mainly for flaming arrows; don't want allies or people in safe zones to be
	// ignited even after damage event is cancelled
	@SuppressWarnings("deprecation")
	@EventHandler(priority = EventPriority.NORMAL, ignoreCancelled = true)
	public void onEntityCombustByEntity(EntityCombustByEntityEvent event) {
		EntityDamageByEntityEvent sub = new EntityDamageByEntityEvent(event.getCombuster(), event.getEntity(),
				EntityDamageEvent.DamageCause.FIRE, 0);
	...
	private static final Set<PotionEffectType> badPotionEffects = new LinkedHashSet<PotionEffectType>(
			Arrays.asList(PotionEffectType.BLINDNESS, PotionEffectType.CONFUSION, PotionEffectType.HARM,
	...
	public void onPotionSplashEvent(PotionSplashEvent event) {
		...
		while (iter.hasNext()) {
			...
			EntityDamageByEntityEvent sub = new EntityDamageByEntityEvent((Entity) thrower, target,
					EntityDamageEvent.DamageCause.CUSTOM, 0);
			if (!this.canDamagerHurtDamagee(sub, true)) {
				event.setIntensity(target, 0.0); // affected entity list doesn't accept modification (so no
													// iter.remove()), but this works
			}
			sub = null;
		}
	}
	...
	public boolean canDamagerHurtDamagee(EntityDamageByEntityEvent sub, boolean notify) {
		...
		// for damage caused by projectiles, getDamager() returns the projectile... what
		// we need to know is the source
		...
		// Players can not take attack damage in a SafeZone, or possibly peaceful
		// territory
		if (defLocFaction.noPvPInTerritory()) {
			if (damager instanceof Player) {
				if (notify) {
					FPlayer attacker = FPlayers.getInstance().getByPlayer((Player) damager);
					attacker.msg(TL.PLAYER_CANTHURT, (defLocFaction.isSafeZone() ? TL.REGION_SAFEZONE.toString()
							: TL.REGION_PEACEFUL.toString()));
				}
				return false;
			}
		...
		/**
		 * if (attacker.hasLoginPvpDisabled()) { if (notify) {
		 * attacker.msg(TL.PLAYER_PVP_LOGIN,
		 * Conf.noPVPDamageToOthersForXSecondsAfterLogin); } return false; }
		 */

		Faction locFaction = Board.getInstance().getFactionAt(new FLocation(attacker));

		// so we know from above that the defender isn't in a safezone... what about the
		// attacker, sneaky dog that he might be?
		if (locFaction.noPvPInTerritory()) {
			if (notify) {
				attacker.msg(TL.PLAYER_CANTHURT,
		...
		/**
		 * if (Conf.disablePVPBetweenNeutralFactions && relation.isNeutral()) { if
		 * (notify) { attacker.msg(TL.PLAYER_PVP_NEUTRAL); } return false; }
		 */
		...
		if (damage > 0.0 && attacker.isInOwnTerritory()) {
			double damageAdd = 0.0D;
			if (attacker.getFaction().getUpgrades().getIncreasedDamage() > 0) {
				if (attacker.getFaction().getUpgrades().getIncreasedDamage() == 1) {
					damageAdd = Math.ceil(damage * 1.05);
					sub.setDamage(damageAdd);
				}

				if (attacker.getFaction().getUpgrades().getIncreasedDamage() == 2) {
					damageAdd = Math.ceil(damage * 1.10);
					sub.setDamage(damageAdd);
				}

				if (attacker.getFaction().getUpgrades().getIncreasedDamage() == 3) {
					damageAdd = Math.ceil(damage * 1.20);
					sub.setDamage(damageAdd);
				}
			}
		}
		...
		/**
		 * if (ownTerritory && relation.isNeutral()) { if (notify) {
		...
		double damageAmount = 0.15;

		if (damage > 0.0 && ownTerritory && Conf.territoryShieldFactor > 0) {
			if (defendFaction.getUpgrades().getReducedDamage() > 0) {
				if (defendFaction.getUpgrades().getReducedDamage() == 1) {
					newDamage = Math.ceil(damage * 0.80);
					damageAmount = 0.20;
				}

				if (defendFaction.getUpgrades().getReducedDamage() == 2) {
					newDamage = Math.ceil(damage * 0.70);
					damageAmount = 0.30;
				}

				if (defendFaction.getUpgrades().getReducedDamage() == 3) {
					newDamage = Math.ceil(damage * 0.50);
					damageAmount = 0.50;
				}

			} else {
				newDamage = Math.ceil(damage * 0.85);
				// Send message
			}
			...
			// Send message
			if (notify) {
				String perc = MessageFormat.format("{0,number,#%}", (damageAmount));
	...
	public void onCreatureSpawn(CreatureSpawnEvent event) {
		...
		if (Conf.safeZoneNerfedCreatureTypes.contains(event.getEntityType())
				&& Board.getInstance().getFactionAt(new FLocation(event.getLocation())).noMonstersInTerritory()) {
			event.setCancelled(true);
		}
	}
	...
	public void onPaintingBreak(HangingBreakEvent event) {
		if (event.getCause() == RemoveCause.EXPLOSION) {
			...
			if ((faction.isWilderness() && !Conf.worldsNoWildernessProtection.contains(loc.getWorld().getName())
					...
					|| (faction.isNormal() && (online
							...
							: (Conf.territoryBlockCreepersWhenOffline || Conf.territoryBlockFireballsWhenOffline
		...
		if (!FactionsBlockListener.playerCanBuildDestroyBlock((Player) breaker, event.getEntity().getLocation(),
				"destroy", false)) {
			event.setCancelled(true);
		}
	}
	...
	public void onPaintingPlace(HangingPlaceEvent event) {
		if (!FactionsBlockListener.playerCanBuildDestroyBlock(event.getPlayer(), event.getBlock().getLocation(),
				"place", false)) {
			event.setCancelled(true);
		}
	}
	...
	public void onEntityChangeBlock(EntityChangeBlockEvent event) {
		...
		if (entity instanceof Enderman) {
			...
			Faction faction = Board.getInstance().getFactionAt(new FLocation(loc));
			// it's a bit crude just using fireball protection, but I'd rather not add in a
			// whole new set of xxxBlockWitherExplosion or whatever
	...
	public void onTravel(PlayerPortalEvent event) {
		...
		if (event.useTravelAgent() && agent.getCanCreatePortal() && agent.findPortal(event.getTo()) == null) {
			...
			String mininumRelation = P.p.getConfig().getString("portals.minimum-relation", "MEMBER"); // Defaults to
																										// Neutral if
																										// typed wrong.
	...
	private boolean stopEndermanBlockManipulation(Location loc) {
		...
		// quick check to see if all Enderman deny options are enabled; if so, no need
		// to check location
