...
public class CmdJoin extends FCommand {
	...
	public void perform() {
		...
		if (faction == fplayer.getFaction()) {
			// TODO:TL
			msg(TL.COMMAND_JOIN_ALREADYMEMBER, fplayer.describeTo(fme, true), (samePlayer ? "are" : "is"),
					faction.getTag(fme));
			return;
		}

		if (Conf.factionMemberLimit > 0) {
			if (faction.getUpgrades().getFactionSize() > 0) {
				if (faction.getUpgrades().getFactionSize() == 1) {
					if (faction.getFPlayers().size() >= 22) {
						msg(TL.COMMAND_JOIN_ATLIMIT, faction.getTag(fme), 17, fplayer.describeTo(fme, false));
						return;
					}
				} else if (faction.getUpgrades().getFactionSize() == 2) {
					if (faction.getFPlayers().size() >= 25) {
						msg(TL.COMMAND_JOIN_ATLIMIT, faction.getTag(fme), 20, fplayer.describeTo(fme, false));
						return;
					}
				} else if (faction.getUpgrades().getFactionSize() == 3) {
					if (faction.getFPlayers().size() >= 30) {
						msg(TL.COMMAND_JOIN_ATLIMIT, faction.getTag(fme), 25, fplayer.describeTo(fme, false));
						return;
					}
				}
			} else {
				if (faction.getFPlayers().size() >= Conf.factionMemberLimit) {
		...
		if (!(faction.getOpen() || faction.isInvited(fplayer) || fme.isAdminBypassing()
		...
		FPlayerJoinEvent joinEvent = new FPlayerJoinEvent(FPlayers.getInstance().getByPlayer(me), faction,
				FPlayerJoinEvent.PlayerJoinReason.COMMAND);
		...
		if (samePlayer && !payForCommand(Conf.econCostJoin, TL.COMMAND_JOIN_TOJOIN.toString(),
				TL.COMMAND_JOIN_FORJOIN.toString())) {
			return;
		}
