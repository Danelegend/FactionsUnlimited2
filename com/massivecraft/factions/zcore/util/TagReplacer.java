...
public enum TagReplacer {
    ...
    FACTION_EXP(TagType.FACTION, "{faction-exp}"),
    FACTION_UPGRADES(TagType.FACTION, "{faction-upgrades}"),
    FACTION_MAXUPGRADES(TagType.FACTION, "{faction-maxupgrades}"),
    FACTION_WEALTH(TagType.FACTION, "{wealth}"),
    ...
    protected String getValue(Faction fac, FPlayer fp) {
        ...
        if (fp != null) {
            switch (this) {
                ...
                case PLAYER_BALANCE:
                    return Econ.isSetup() ? Econ.getFriendlyBalance(fp) : TL.ECON_OFF.format("player-balance");
                case PLAYER_POWER:
        ...
        switch (this) {
            ...
            case SHIELD:
                return fac.hasShield() ? ChatColor.GREEN + "true" : ChatColor.RED + "false";
            case RAIDABLE:
            ...
            case FACTION_POINTS:
                return NumberUtil.format(fac.getPoints());
            case FACTION_EXP:
                return String.valueOf(fac.getTotalXP());
            case FACTION_UPGRADES:
                return String.valueOf(fac.getUpgrades().getTotalAmount());
            case FACTION_MAXUPGRADES:
                return String.valueOf(fac.getUpgrades().getMaxAmount());
            case FACTION_WEALTH:
            	return String.valueOf(fac.getFactionWealth());
        }
        return null;
    }
